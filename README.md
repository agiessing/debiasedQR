...
- ``debiasedPredict()`` Wrapper function which takes either `.drq` or `.drqcv` objects and returns two debiased estimates of the conditional quantile function (one based on the primal variable and another based on the dual variable), the biased $\ell_1$-penalized pilot estimate of the conditional quantile function, and an estimate of the asymptotic variance of the debiased estimate. When applied to a `.drqcv` object, the wrapper function first finds the optimal tuning parameter $\gamma^* > 0$ based on cross-validated dual losses.
