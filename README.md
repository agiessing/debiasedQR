...
- ``drqcv()`` Solves primal and dual rank-score debiasing programs for a range of tuning parameters and returns the values of the optimal cross-validated dual losses, which can be used to determine the optimal tuning parameter $\gamma^* > 0$. Since cross-validation is computationally expensive, we provide the option to parallelize the computation. 
- ``debiasedPredict()`` Wrapper function which takes either `.drq` or `.drqcv` objects and returns two debiased estimates of the conditional quantile function (based on solutions of either the primal or the dual program), the biased $\ell_1$-penalized pilot estimate of the conditional quantile function, and an estimate of the asymptotic variance of the debiased estimate. When applied to a `.drqcv` object, the wrapper function first finds the optimal tuning parameter $\gamma^* > 0$ based on cross-validated dual losses.
