...
dualADMM <- function(X, x, Psi=NULL, v_init=NULL, gamma = 0.2, rho=5, max_iter = 500,
  ...
  while( (cnt <= max_iter) && ( (res_primal >= eps_primal) || (res_dual >= eps_dual) ) ) {
    ...
    eps_primal <- sqrt(d) * tol[1] + tol[2] * max( norm(v_new, type = "2"),
                                                   norm(z_new, type ="2") )
    eps_dual <- sqrt(d) * tol[1] + tol[2] * rho * norm(u_new, type ="2")

    if(!quiet){
      message(paste0("ADMM step count: ", cnt, ". Primal residual criterion: ",
                    res_primal - eps_primal, ". Dual residual criterion: ",
                    res_dual - eps_dual, "\n"))
    }

    if ( (cnt > max_iter) ) {
      message(paste0("ADMM algorithm has reached max. no. of iterations for gamma = ",
                     round(gamma, 4),"."))
      }
    }
